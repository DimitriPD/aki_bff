...
/**
 * Aggregated use case for class details
 * Combines data from Personas (class, students, teachers) and Core (events, stats)
 */
export class GetClassDetailsUseCase {
  ...
  async execute(classId: number, correlationId: string) {
    ...
    try {
      // Fetch class with members from Personas
      const classWithMembers = await this.personasClient.getClassWithMembers(classId);

      // Fetch events for this class from Core (recent + upcoming)
      ...
      // Get attendance stats for the class (from all events)
      ...
      if (recentEvents.items.length > 0) {
        ...
        averageAttendance = attendanceCounts.length > 0
          ? Math.round((totalAttendances / attendanceCounts.length / totalStudents) * 100)
          : 0;
      }
