...
import { IFunctionNotificationClient } from './types';
...
export class FunctionNotificationClient implements IFunctionNotificationClient {
  ...
  async sendNotification(data: { class_id: number; teacher_id: number; message: string }): Promise<{ status: string; notification_id: number }> {
    return this.http.post<{ status: string; notification_id: number }>("/api/notification", data);
  }
}
