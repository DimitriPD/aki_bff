...
/**
 * Vertical Slice: Get Student Profile
 * Aggregates data from Personas (student + classes) and Core (attendances + events) into a single response.
 */
export class GetStudentProfileUseCase {
  ...
  async execute(studentId: number, correlationId: string) {
    ...
    try {
      ...
      // Fetch all classes to find which ones the student belongs to (TODO: optimize with server-side filter when API supports it)
